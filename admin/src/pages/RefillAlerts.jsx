  ...
  useEffect(() => {
    if (alertData) {
      const mapped = alertData.map(a => ({
        ...
        status: a.isOverdue ? 'OVERDUE' : (a.medicineId?.stock === 0 ? 'EMPTY' : 'LOW'),
        daysLeft: a.daysLeft,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {alerts.map(alert => (
            <div className="card" key={alert._id} style={{ padding: '18px 22px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', alignItems: 'center', gap: 20 }}>
                ...
                <div>
                  <div style={{
                    ...
                    color: (alert.daysLeft <= 3 || alert.isOverdue) ? 'var(--accent-red)' : 'var(--accent-orange)'
                  }}>
                    {alert.isOverdue ? '0 days' : `${alert.daysLeft} days`}
                  </div>
                  <div style={{ fontSize: 12, color: alert.isOverdue ? 'var(--accent-red)' : 'var(--text-muted)', fontWeight: alert.isOverdue ? 700 : 400 }}>
                    {alert.isOverdue ? '⚠️ OVERDUE' : 'until out of stock'}
                  </div>
                </div>
              ...
              {(alert.daysLeft <= 3 || alert.isOverdue) && (
                <div style={{
                  ...
                  {alert.isOverdue ?
                    `Urgent: Patient is OUT OF MEDICINE! Predicted refill was ${alert.daysLeft} days ago.` :
